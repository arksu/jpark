...
import org.jpark.helper.ClassConstants;
import org.jpark.helper.Helper;
...
import static org.jpark.helper.ClassConstants.*;
...
//				statement.setNString(index, (String) parameter);
		...
		{
			statement.setTimestamp(index, Timestamp.valueOf((java.time.LocalDateTime) parameter));
		}
		else if (parameter instanceof java.time.OffsetDateTime)
		{
			statement.setTimestamp(index, Timestamp.from(((java.time.OffsetDateTime) parameter).toInstant()));
		}
...
//			}
		...
		{
			statement.setTimestamp(index, Helper.timestampFromDate((Date) parameter));
		}
...
//		}
	...
	public static Object getObjectThroughOptimizedDataConversion(ResultSet resultSet, DatabaseField field, int columnNumber) throws SQLException
	{
		...
		else if ((type == Types.TIME) || (type == Types.DATE) || (type == Types.TIMESTAMP))
		{
			...
			{
				final Timestamp ts = resultSet.getTimestamp(columnNumber);
